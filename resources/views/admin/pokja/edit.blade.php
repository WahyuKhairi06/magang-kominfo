...
<div class="p-6">
  ...
  <div class="max-w-3xl mx-auto bg-white rounded-3xl shadow-lg border border-slate-200 overflow-hidden">
    ...
    <div class="p-6 border-b flex items-center justify-between">
      <h3 class="font-bold text-slate-800 text-lg">Edit Pokja</h3>
    ...
    <!-- FORM -->
    <form action="{{ route('pokja.update', $data->id) }}" method="POST" class="p-6 space-y-6">
      @csrf
      @method('PUT')
      ...
      <div>
        ...
        <input type="text" name="nama_pokja"
          value="{{ old('nama_pokja', $data->nama_pokja) }}"
      ...
      <div>
        ...
        <textarea name="keterangan" rows="4"
          class="w-full rounded-xl border border-slate-300 px-4 py-2 focus:ring-2 focus:ring-primary focus:outline-none"
          placeholder="Contoh: Penghayatan dan Pengamalan Pancasila">{{ old('keterangan', $data->keterangan) }}</textarea>
      </div>
