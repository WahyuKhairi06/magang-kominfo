<div class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md modal-overlay" id="modal-edit{{ $role->id }}">
  <div class="bg-white w-full max-w-2xl rounded-[32px] shadow-2xl overflow-hidden modal-content">
    ...
    <div class="p-8 pb-4 flex justify-between items-start border-b">
      ...
      <button 
        ...
        onclick="closeModal('modal-edit{{ $role->id }}')">
        <span class="material-symbols-outlined">close</span>
      </button>
    </div>

    <!-- FORM -->
    <form class="p-8 space-y-6" action="{{ url('role').'/'. $role->id }}" method="POST">
      @csrf
      @method('PUT')
