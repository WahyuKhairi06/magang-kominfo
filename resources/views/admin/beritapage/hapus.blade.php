<div class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md modal-overlay"
    ...
    <div class="bg-white w-full max-w-md rounded-[24px] shadow-2xl overflow-hidden modal-content p-6">
        ...
        <p class="text-sm text-slate-500 text-center mb-6">
            Data <b>{{ $item->judul }}</b> akan dihapus permanen dan tidak bisa dikembalikan.
        </p>
        ...
        <div class="flex gap-3">
            ...
            <button type="button"
                ...
                onclick="closeModal('modal-delete{{ $item->id }}')">
                Batal
            </button>

            <form action="{{ route('beritapage.destroy', $item->id) }}" method="POST" class="flex-1">
                @csrf
                @method('DELETE')
